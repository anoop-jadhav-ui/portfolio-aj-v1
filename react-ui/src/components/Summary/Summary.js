...
class Summary extends React.Component {
  ...
  constructor(props){
    ...
    this.mainLogoRef = createRef();
    this.rightPane = createRef();
    this.containerRef = createRef()
    this.centerPos = createRef();
    
  }

  state = {
    mainLogoUrl: "",
    mainLogoUpdateClass: false
  };

  
  componentDidMount(){

    this.rightPane.current = document.querySelector('.horizontally-placed');
    let container = this.containerRef.current.getBoundingClientRect();
    let mainLogoBounds = this.mainLogoRef.current.getBoundingClientRect();
    // this.centerPos.current = (container.left + (container.width / 2)) - (mainLogoBounds.width / 2) ;

  }

  componentDidUpdate() {
   if(this.rightPane.current){
    var rect = this.rightPane.current.getBoundingClientRect();

    if (this.props.scrollPos > rect.y) {
      this.mainLogoRef.current.classList.add('moveToRightPane');
      this.rightPane.current.classList.add('increaseheight')

    }else{
      this.mainLogoRef.current.classList.remove('moveToRightPane');
      this.rightPane.current.classList.remove('increaseheight')
    }
   }
  }
  ...
  render() {
    return (
      <div
        className={
          "show-on-scroll col-md-7 page-1 text-center summary " + this.props.class
        }
      >
        ...
        <div className="mainLogoWrapper" ref={this.containerRef}>
          <svg ref={this.mainLogoRef} style={{left : `${this.centerPos.current}px`}} className="mainlogo" width="196" height="313" viewBox="0 0 196 313" fill="none" xmlns="http://www.w3.org/2000/svg">
